...
export const HeaderSearchComponent: React.FC<IHeaderSearchComponentProps> = ({ 
  ...
  if (formik.values.name) {
    if (isLoading) {
      collapseContent = <Spinner size='md' color='#349a89' className={styles.spinner} />;
    } else {
      if (!suggestions.length) {
        collapseContent = <p className={styles.noSuggestions}>No suggestions</p>;
      } else {
      ...
      <form onSubmit={formik.handleSubmit} className={styles.search}>
        <div className={styles.menu} ref={ref} id='menu'>
          <InputField 
            ...
            name='name'
            wrapperClassName={[styles.nameInput, isOpen ? styles.openRecent : ''].join(' ')}
