...
export const EditProfile: React.FC = () => {
  ...
  const onChange = (event: React.SyntheticEvent<HTMLInputElement>): void => {
    if (event.currentTarget?.files &&
      ...
      event.currentTarget?.files[0]?.type !== 'image/png') {
        dispatch(sendNotificationError('File type does not match .jpeg, .jpg or .png'));
    } else {
      if (event.currentTarget.files !== null) {
        ...
        axios.post('/upload/images', formData, {
          ...
          .then((res) => {
            dispatch(sendNotificationSuccess('New photo uploaded successfully'));
            setAvatar(res.data);
  ...
  const onSubmit = (values: FormikValues): void => {
    dispatch(userUpdate(values.fullName, avatar, values.phone, values.location));
  }
  ...
  const validate = (values: IFormikValues) => {
    ...
    } else if (!/^((\\+[1-9]{1,4}[ \\-]*)|(\\([0-9]{2,3}\\)[ \\-]*)|([0-9]{2,4})[ \\-]*)*?[0-9]{3,4}?[ \\-]*[0-9]{3,4}?$/i.test(values.phone)) {
